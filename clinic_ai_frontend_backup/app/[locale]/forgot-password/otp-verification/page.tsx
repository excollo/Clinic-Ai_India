...
export default function ForgotPasswordOtpVerificationPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    localStorage.setItem('password_reset_otp', otp.trim());
    setTimeout(() => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-cream-50 via-white to-sand-50 flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-xl p-8 border border-slate-200">
          <h1 className="text-2xl font-bold text-slate-900 mb-2">OTP Verification</h1>
          <p className="text-slate-600 text-sm mb-6">
            Enter the OTP sent to {email || 'your email'} to continue resetting your password.
          </p>
